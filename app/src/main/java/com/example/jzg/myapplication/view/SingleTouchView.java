...
public class SingleTouchView extends View {
	...
	public static final int DEFAULT_FRAME_COLOR = Color.TRANSPARENT;
	...
	private void init(){
		...
		mPaintBitMap.setStrokeWidth(frameWidth);
		mPaintBitMap.setStyle(Style.STROKE);
...
//			layout(newPaddingLeft, newPaddingTop, newPaddingLeft + actualWidth, newPaddingTop + actualHeight);
		...
		switch (event.getAction() ) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mStatus == STATUS_ROTATE_ZOOM) {
					...
					int wh = halfBitmapWidth * halfBitmapWidth + halfBitmapHeight * halfBitmapHeight;
					double d = Double.parseDouble(String.valueOf(wh)) ;
					//图片某个点到图片中心的距离
					double bitmapToCenterDistance = Math.sqrt(d);

					//移动的点到图片中心的距离
					double moveToCenterDistance = distance4PointF(mCenterPoint, mCurMovePointF);
	...
	private int JudgeStatus(float x, float y){
		...
		//点击的点到控制旋转，缩放点的距离
		double distanceToControl = distance4PointF(touchPoint, controlPointF);
	...
	private double distance4PointF(PointF pf1, PointF pf2) {
		...
		float disxy = disX * disX + disY * disY;
		double d = Double.parseDouble(String.valueOf(disxy)) ;
		return Math.sqrt(d);
	}
